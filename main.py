...
app=FastAPI(title="Movie Recommender API",version="1.0")  #App created
...
app.add_middleware(
    ...
    allow_methods=["*"],
    allow_headers=["*"],
)    # Allows to access from frontend
...
async def tmdb_get(path: str, params: Dict[str, Any], retries: int = 2) -> Dict[str, Any]:
    """
    Safe TMDB GET with a shared client and a short retry for transient failures.
    """
    q = dict(params)
    ...
    for attempt in range(retries + 1):
        try:
            r = await httpx_client.get(f"{TMDB_BASE}{path}", params=q)
        except httpx.RequestError as e:
            last_error = f"TMDB request error:{type(e).__name__} | {repr(e)}"
        ...
        if r.status_code in (429, 500, 502, 503, 504) and attempt < retries:
            last_error = f"TMDB error{r.status_code}:{r.text}"
            await asyncio.sleep(0.4 * (attempt + 1))
            continue

        raise HTTPException(status_code=502, detail=f"TMDB error{r.status_code}:{r.text}")

    raise HTTPException(status_code=502, detail=last_error or "TMDB request failed after retries")
...
def build_title_to_idx_map(indices:Any)->Dict[str,int]:
    ...
    indices.pkl can be:
    -dict(title->index)
    -pandas series (index=title, valu=index)
...
def tfidf_recommend_titles(query_title:str,top_n:int=10)->List[Tuple[str,float]]:
    """
    Returns list of(title,score) from local df using cosine similarity on TF-IDF martrix
...
def load_pickles():
    ...
    # load tf-idf (usually spicy sparse)
...
# bundle: details + tfidf rec + genre rec ( sum of above 3 func)
...
async def search_bundle(
    ...
    """
    When a selected a movie and want:
    - movie details
    - tf-idf recommendations + posters
    - genre recommendations + posters
    NOTE: 
    - selects the best match from tmdb
    - i want multiple matches use tmdb/search
    """
    ...
    if details.genres:
        ...
        genre_cards = await tmdb_cards_from_results(
            discover.get("results", []),
            limit=genre_limit,
        )

        genre_cards = [
            c for c in genre_cards
    ...
    return SearchBundleResponse(
        ...
        genre_recommendation=genre_cards,
    )